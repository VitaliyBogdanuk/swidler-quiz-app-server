...
const phoneDescriptionController = require('../controllers/phoneDescriptionController');
...
router.get('/tables/phoneDescriptions', ensureAuthenticated, async (req, res) => {
    try {
        const phoneDescriptionsList = await phoneDescriptionController.getPhoneDescriptions();
        res.render('pages/phoneDescriptions', { user: req.user, phoneDescriptionsList: phoneDescriptionsList });
    } catch (error) {
        // handle error
        console.log(error);
        req.flash('error', error);
